 ...
 * @author      Timma Meyatchie Dieudonne, <dieudonnetimmameyatchie@mni.thm.de>
 ...
 * @since     v1.2.0
 */
class THM_Listview extends THM_GroupsData
{
	/**
	 ...
	 */
	public static function getListAll($params, $pagetitle, $gid)
	{
		...
		$rows            = THM_GroupsData::getUserCount($groupid);
		...
		$allLastNames = THM_GroupsData::getFirstletter($groupid);
		...
		$attribut = THM_GroupsData::getUrl(array("name", "gsuid", "gsgid"));
		...
		{
			$char               = $abc[$i];
			$rows               = THM_GroupsData::getUserByLetter($groupid, $char);
...
// 	    echo $result .'</div>';
	 ...
	 */
	public static function getListAlphabet($params, $pagetitle, $gid)
	{
		$scriptDir = JUri::root() . "media/com_thm_groups/js/";
		...
		$attribut = THM_GroupsData::getUrl(array("name", "gsuid", "gsgid", "letter", "groupid"));
		...
		$retString .= THM_Listview::getUserForLetter(
			$groupid,
			$params['columnCount'],
			$shownLetter,
			$paramLinkTarget,
	 ...
	 */
	public static function getUserForLetter($gid, $column, $letter, $paramLinkTarget, $orderAttr, $showStructure, $linkElement, $oldattrinut)
	{
		$retString = '<div id="new_user_list">';
		$retString .= "<ul><br /><br />";

		$groupMember = THM_GroupsData::getGroupMemberByLetter($gid, $letter);
		$memberWithU = array();
		...
		foreach ($groupMember as $member)
		{
			...
			else
			{
				...
				$retString .= THM_Listview::writeName($arrOrderAtt, $member, $showStructure, $linkElement, $linkTarget, $groupid);
				$actualRowPlaced++;
			}
	 ...
	 * @return  String  $string  Return String
	 */
	public static function writeName($arrOrderAtt, $member, $arrshowStructure, $linkElement, $linkTarget, $groupid)
	{
	 ...
	 * @return  String   $result  the HTML code of te view
	 */
	public static function getCssView($params)
	{
