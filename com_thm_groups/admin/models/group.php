...
class THM_GroupsModelGroup extends JModelLegacy
{
	...
	private function associateRole($roleID, $groupID)
	{
		...
		$insertQuery->insert('#__thm_groups_usergroups_roles')->columns(array('usergroupsID', 'rolesID'));
	...
	private function associateTemplate($templateID, $groupID)
	{
		...
		// First, we need to check if the profile is already assigned to a group
		$existingQuery->select('profileID')->from('#__thm_groups_profile_usergroups')->where("usergroupsID = '$groupID'");
		$this->_db->setQuery($existingQuery);
		...
		if (empty($existingTemplateID))
		{
			...
			$insertQuery->insert('#__thm_groups_profile_usergroups')->columns(array('usergroupsID', 'profileID'));
		...
		else
		{
			...
			$updateQuery->update('#__thm_groups_profile_usergroups')
	...
	public function removeRole($roleID = 0, $groupID = 0)
	{
		...
		$query->delete('#__thm_groups_usergroups_roles')->where("rolesID = '$roleID'")->where("usergroupsID = '$groupID'");
