...
SELECT '' AS ID, users.id AS usersID, cat.id AS categoriesID 
FROM #__thm_groups_users AS users 
JOIN #__categories AS cat 
ON users.id = cat.created_user_id AND cat.parent_id = (SELECT id FROM #__categories WHERE path = "quickpages");
...
ENGINE = InnoDB;
 ...
 SELECT '' AS ID, users.id AS usersID, content.id AS contentID, '' AS featured
 FROM #__thm_groups_users AS users
 JOIN #__content AS content
 ON users.id = content.created_by AND content.catid IN (SELECT categoriesID FROM #__thm_groups_user_categories);
...
SELECT struct.id AS id, dyntype.id AS dynamic_typeID, struct.field AS name
FROM #__thm_groups_structure AS struct
JOIN #__thm_groups_dynamic_type AS dyntype
ON struct.type = dyntype.name;

CREATE TABLE IF NOT EXISTS `jos_thm_groups_users_attributes` (
  `ID` INT(11) NOT NULL AUTO_INCREMENT,
  `usersID` INT(11) NOT NULL,
  ...
  ON UPDATE CASCADE
  ON DELETE CASCADE,
  FOREIGN KEY (`attibuteID`) REFERENCES `#__thm_groups_attributes` (`id`)
  ON UPDATE CASCADE
  ON DELETE CASCADE)
ENGINE = InnoDB; 

INSERT INTO #__thm_groups_users_attributes(userID, attributeID, value, published)
SELECT userid, structid AS attributeID, value, publish AS published
...
SELECT map.uid as userID, map.gid as usergroupsID, map.rid AS rolesID
...
SELECT 1, structitem.id AS attributeID, structure.order AS orderBy
