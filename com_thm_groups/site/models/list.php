...
class THMGroupsModelList extends JModel
{
    ...
    public function getgListAll()
    {
        ...
        $queryGetDiffLettersToFirstletter = "SELECT distinct t.value as lastName " . "FROM `#__thm_groups_text` as t , " . "`#__thm_groups_additional_userdata` as ud, " . "`#__thm_groups_groups_map` as gm " . "where t.structid = 2 and t.userid = ud.userid and ud.published = 1 and t.userid = gm.uid and gm.gid=$groupid and gm.rid != 2";
        ...
        foreach ($abc as $char) {
            ...
            $query = "SELECT distinct b.userid as id, " . "b.value as firstName, " . "c.value as lastName, " . "d.value as EMail, " . "e.value as userName, " . "f.usertype as usertype, " . "f.published as published, " . "f.injoomla as injoomla, " . "t.value as title " . "FROM `#__thm_groups_structure` as a " . "inner join #__thm_groups_text as b on a.id = b.structid and b.structid=1 " . "inner join #__thm_groups_text as c on b.userid=c.userid and c.structid=2 " . "inner join #__thm_groups_text as d on c.userid=d.userid and d.structid=3 " . "inner join #__thm_groups_text as e on d.userid=e.userid and e.structid=4 " . "left outer join #__thm_groups_text as t on e.userid=t.userid and t.structid=5 " . "inner join #__thm_groups_additional_userdata as f on f.userid = e.userid, " . "`#__thm_groups_groups_map` " . "WHERE published = 1 and c.value like '$char%' and e.userid = uid and gid = $groupid " . "ORDER BY lastName";
            $db->setQuery($query);
            //$db->query();
            $rows = $db->loadObjectList();
            if (sizeof($rows) <= 0) {
            ...
            //Umbruch nÃƒÂ¶tig
            else if ((sizeof($rows) + $placedChar) > $maxColumnSize) {
                ...
                else if (sizeof($rows) + $placedChar == $maxColumnSize - 2 && sizeof($rows) == 3) {
                    $placedChar = 1;
                    $retString .= "</div><div id='row_" . $rowCount . "_column_" . $columnCount . "_max_" . $maxColumnSize . "' $divStyle>"; //ist  das div richtig so?
                    $retString .= "<ul class='alphabet'>";
                    $retString .= "<a class='list' margin-bottom:" . $margin . "px;\">" . $char;
                ...
                else if ($placedChar <= $maxColumnSize - 2 && $placedChar + sizeof($rows) == $maxColumnSize + 1 && sizeof($rows) > 3) {
                    $retString .= "<ul class='alphabet'>";
                    $retString .= "<a class='list' margin-bottom:" . $margin . "px;\">" . $char;
                    $retString .= "</a>";
                    foreach ($rows as $row) {
                ...
                else if ((sizeof($rows) + $placedChar) >= $maxColumnSize + 2) {
                    $retString .= "<ul class='alphabet'>";
                    $retString .= "<a class='list' margin-bottom:" . $margin . "px;\">" . $char;
                    $retString .= "</a>";
                    foreach ($rows as $row) {
    ...
    public function getgListAlphabet()
    {
        ...
        $query = "SELECT distinct b.userid as id, " . "b.value as firstName, " . "c.value as lastName, " . "d.value as EMail, " . "e.value as userName, " . "f.usertype as usertype, " . "f.published as published, " . "f.injoomla as injoomla, " . "t.value as title " . "FROM `#__thm_groups_structure` as a " . "inner join #__thm_groups_text as b on a.id = b.structid and b.structid=1 " . "inner join #__thm_groups_text as c on b.userid=c.userid and c.structid=2 " . "inner join #__thm_groups_text as d on c.userid=d.userid and d.structid=3 " . "inner join #__thm_groups_text as e on d.userid=e.userid and e.structid=4 " . "left outer join #__thm_groups_text as t on e.userid=t.userid and t.structid=5 " . "inner join #__thm_groups_additional_userdata as f on f.userid = e.userid, " . "`#__thm_groups_groups_map` " . "WHERE published = 1 and c.value like '$shownLetter%' and e.userid = uid and gid = $groupid " . "ORDER BY lastName";
