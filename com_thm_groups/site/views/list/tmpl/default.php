 ...
 *
 * @param Array $params
 *        	contain the Paramter for the View
...
function getListAll($params, $pagetitle, $gid) {
    ...
    $attribut = THMLibThmGroups::getUrl ( array (
            "name",
            "gsuid",
            "gsgid"
    ) );
 ...
 *
 * @param Array $params
 *        	Contains the Paramter for the View
 * @param String $pagetitle
 *        	Title of teh page
 * @param Integer $gid
 *        	Contain the group Id
...
function getListAlphabet($params, $pagetitle, $gid) {
    ...
    $retString .= getUserForLetter ( $groupid, $params ['columnCount'], $shownLetter, $paramLinkTarget, $orderAttr, $showStructure, $linkElement, $attribut );
 ...
 *
 * @param Integer $gid
 *        	String of the Attributes order
 * @param Integer $column
 *        	Array of the attributes order
 * @param Integer $letter
 *        	Data of the user
 * @param String $paramLinkTarget
 *        	contain module oder profile
 * @param String $orderAttr
 *        	contain the Order of Attribute
 * @param String $showStructure
 *        	is the title on
 * @param String $linkElement
 *        	the Element of the link
 * @param String $oldattrinut
 *        	contain the all url attribut for the Breadscrum modul
 *
...
function getUserForLetter($gid, $column, $letter, $paramLinkTarget, $orderAttr, $showStructure, $linkElement, $oldattrinut) {
    ...
    switch ($paramLinkTarget) {
        case "module" :
            ...
            $linkTarget = 'index.php?option=com_thm_groups&view=list&layout=default&' . $oldattrinut . 'Itemid=' . $itemid . '&groupid=' . $groupid . '&letter=' . $letter;
            break;
        case "profile" :
            $linkTarget = 'index.php?option=com_thm_groups&view=profile&layout=default&' . $oldattrinut . 'Itemid=' . $itemid . '&pageTitle=' . rawurlencode ( $pagetitle );
            break;
        default :
            $linkTarget = 'index.php?option=com_thm_groups&view=list&layout=default&' . $oldattrinut . 'Itemid=' . $itemid . '&groupid=' . $groupid . '&letter=' . $letter;
    }
 ...
 *
 * @param object $arrOrderAtt
 *        	Array of the attributes order
 * @param object $member
 *        	Data of the user
 * @param Array $arrshowStructure
 *        	Show the title of the users
 * @param String $linkElement
 *        	The Elements where a link should place
            ...
            switch ($val) {
                case '1' :
                    if (array_search ( '0', $arrshowStructure ) !== null && array_search ( '0', $arrshowStructure ) !== false) {
                        if (! empty ( $linkElement ) && array_search ( '0', $linkElement ) !== null && array_search ( '0', $linkElement ) !== false) {
                            $string .= '<a href="' . JRoute::_ ( $linkTarget . '&gsuid=' . $member->id . '&name=' . trim ( $member->lastName ) . '&gsgid=' . $groupid ) . '">';
                ...
                case '2' :
                    if (array_search ( '1', $arrshowStructure ) !== null && array_search ( '1', $arrshowStructure ) !== false) {
                        ...
                        if (($run == 0 && array_search ( '2', $arrshowStructure ) !== false) || ($run == 1 && $showTitle == 1 && $arrOrderAtt [2] == 1) || ($run == 2 && $member->posttitle)) {
                            $string .= ',';
                        }
                ...
                case '3' :
                    if (array_search ( '2', $arrshowStructure ) !== null && array_search ( '2', $arrshowStructure ) !== false) {
                        ...
                        if ($run == 0 || ($run == 1 && $showTitle == 1 && $member->title != '') || ($run == 1 && $showTitle == 0 && $arrOrderAtt [0] == "1") || ($run == 2 && ! empty ( $member->posttitle ))) {
                            $string .= ',';
                        }
