...
<form action="<?php echo JRoute::_('index.php?option=com_thm_groups&view=articles' . $itemParam); ?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		...
		<div class="qp_filter_search">
			...
			<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" title="<?php echo JText::_('COM_THM_QUICKPAGES_FILTER_DESC'); ?>" />
				...
				<th width="15%">
					...
					<?php
						if ($canCreate AND $currCategoryID != 0)
						{
							...
							$editURL = JRoute::_('index.php?option=com_content&view=form&layout=edit&catid=' . $currCategoryID . $itemParam . $staticParams);

							$imgSpanTag = '<span class="qp_icon_big qp_create_icon"><span class="qp_invisible_text">New</span></span>';

							echo JHTML::_('link', $editURL, $imgSpanTag, 'title="' . JText::_('COM_THM_QUICKPAGES_HTML_CREATE') . '" class="qp_icon_link"');
						}
						else
						{
				...
				<td class="center">
					<?php
						...
						if ($canEdit)
						{
							...
							echo JHTML::_('link', $editURL, $imgSpanTag, 'title="' . JText::_('COM_THM_QUICKPAGES_HTML_EDIT_ITEM') . '" class="jgrid"');
							echo "\n";
						}
						else
						{
						...
						if ($item->state >= 0)
						{
							...
							$states	= array(
								...
								3	=> array('trash',	'JPUBLISHED',	'COM_THM_QUICKPAGES_HTML_TRASH_ITEM',	'JPUBLISHED',	false,	'trash',	'trash_disabled'),
								-3	=> array('publish',	'JTRASHED',		'COM_THM_QUICKPAGES_HTML_UNTRASH_ITEM',	'JTRASHED',		false,	'untrash',	'untrash'),
							);
